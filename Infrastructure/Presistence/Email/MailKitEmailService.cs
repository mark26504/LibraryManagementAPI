namespace LibraryManagement.Persistence.Email
{
    internal sealed class MailKitEmailService : IEmailService
    {
        ...
        {
            var callbackUrl = BuildCallbackUrl("confirm-email", email, token);
        ...
        {
            var callbackUrl = BuildCallbackUrl("reset-password", email, token);
        ...
        private string BuildCallbackUrl(string route, string email, string token)
            => $"{_options.FrontendBaseUrl.TrimEnd('/')}/auth/{route}" +
        ...
        private async Task SendAsync(string to, string subject, string htmlBody)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Email sent to {Email} with subject {Subject}", to, subject);
            }
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to send email to {Email} with subject {Subject}", to, subject);
            }
        }
    }
}